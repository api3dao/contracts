...
Types are also exported and can be found in `src/types.ts`. Contract-related types are also generated by TypeChain and can be found in `typechain-types` after building the project
...
const address = computeCommunalApi3ReaderProxyV1Address({
  chainId: '1',
  dapiName: 'ETH/USD',
});
```
...
const dappId = unsafeComputeDappId({
  dappAlias: 'dtrinity',
  chainId: '252',
});
```
...
const address = computeDappSpecificApi3ReaderProxyV1Address({
  dappAlias: 'dtrinity',
  chainId: '252',
  dapiName: 'BTC/USD',
});
