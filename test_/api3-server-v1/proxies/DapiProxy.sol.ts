...
describe('DapiProxy', function () {
  async function deploy() {
    ...
    const dapiProxy = await dapiProxyFactory.deploy(await api3ServerV1.getAddress(), dapiNameHash);
  ...
  describe('read', function () {
    context('dAPI name is set', function () {
      ...
      context('Data feed is not initialized', function () {
        it('reverts', async function () {
          ...
          const dapiProxyFactory = await ethers.getContractFactory('DapiProxy', roles.deployer);
          const dapiProxy = await dapiProxyFactory.deploy(await api3ServerV1.getAddress(), uninitializedDapiNameHash);
    ...
    context('dAPI name is not set', function () {
      it('reverts', async function () {
        ...
        const dapiProxyFactory = await ethers.getContractFactory('DapiProxy', roles.deployer);
        const dapiProxy = await dapiProxyFactory.deploy(await api3ServerV1.getAddress(), unsetDapiNameHash);
