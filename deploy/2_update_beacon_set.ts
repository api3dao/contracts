...
const EXAMPLE_AIRNODE_ADDRESS = getAddress('0x07b589f06bD0A5324c4E2376d66d2F4F25921DE1');
const BEACON_SET_BEACON_COUNT = 7;

async function signData(deployer: Signer, templateId: BytesLike, timestamp: number, value: number): Promise<BytesLike> {
...
export default deployScript(
  async (env) => {
    const signer: Signer = await new BrowserProvider(env.network.provider as never).getSigner(
    ...
    // If the deployer account is the expected deployer address, it will do both (1) and (2).
    ...
    const api3ServerV1 = Api3ServerV1__factory.connect(Api3ServerV1.address, signer);
    ...
    const beaconIds = templateIds.map((templateId) =>
      solidityPackedKeccak256(['address', 'bytes32'], [EXAMPLE_AIRNODE_ADDRESS, templateId])
    );
    const beaconSetId = keccak256(AbiCoder.defaultAbiCoder().encode(['bytes32[]'], [beaconIds]));
    ...
    const initialValue = AbiCoder.defaultAbiCoder().encode(['int224', 'uint32'], [0, 0]);
  ...
  for (const [ind, templateId] of templateIds.entries()) {
    if (dataFeedReadings[ind] === initialValue) {
      const signature = await signData(signer, templateId, ind + 1);
      ...
    }
  }
  ...
  */
    ...
    if (updateMulticallData.length > 0) {
      ...
      env.showMessage('Executed Beacon set update');
    } else {
      env.showMessage('Beacon set update already executed');
    }

    if (EXAMPLE_AIRNODE_ADDRESS !== (await signer.getAddress())) {
      env.showMessage('Skipping gas estimation for non-expected deployer');
      return;
    }
    ...
    for (const [ind, templateId] of templateIds.entries()) {
      const signature = await signData(signer, templateId, ind + 102, ind + 102);
    ...
    // env.showMessage(`Beacon set update estimate gas calldata:\n${estimateGasCalldata}`);
    const voidSigner = new VoidSigner(EXAMPLE_AIRNODE_ADDRESS, signer.provider);
    const gasCost = await voidSigner?.estimateGas({ to: api3ServerV1.getAddress(), data: estimateGasCalldata });
    env.showMessage(`Estimated Beacon set update gas cost: ${gasCost}`);
  },
  { tags: ['update'] }
);
