...
module.exports = async () => {
  ...
  if (chainsSupportedByDapis.includes(network.name)) {
    ...
    if (!managerMultisigAddress) {
      throw new Error(`Manager multisig address missing for ${network.name}`);
    }
    ...
    const proxyFactory = new ethers.Contract(proxyFactoryAddress, proxyFactoryAbi, deployer) as unknown as ProxyFactory;
